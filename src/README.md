...
✅ 用户登录时会校验手机号和验证码，根据手机号到数据库查询用户信息，如果用户不存在则会新建用户。然后生成token作为Redis的key，当校验用户是否登录时会携带token进行访问，从redis中取出token进行校验，判断是否存在此数据，如果不存在则会拦截，如果存在则存入ThreadLocal，放行。
...
✅ 当查询请求到达时，会先到缓存中查询，如果缓存中不存在数据，则到数据库中查询，并将结果写入缓存中，设置超时时间。当修改请求到达时，先修改数据库，再删除缓存，等待下一次查询时再写入缓存。
...
### 使用Redis的 ZSet 数据结构实现了点赞排行榜功能,使用Set 集合实现关注、共同关注、Feed流推送功能
...
✅ Feed流是指当用户关注其他用户后，此用户发布内容会推送给关注者，以提供一种持续的、沉浸式的体育，通过无线下拉获取新的信息。

✨ 常见Feed流模式：

- Timeline模式：不进行内容筛选，按照内容发布时间线推送。  
- 智能排序模式：利用算法屏蔽违规和用户不感兴趣的内容，推送用户感兴趣的内容。
...
来组  [🎁CSDN-美团首次面试项目话术1——黑马点评篇](https://blog.csdn.net/m0_68657832/article/details/139177872)
